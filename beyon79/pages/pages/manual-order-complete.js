...
export default function ManualOrderPage() {
  ...
  const stopCategoryAutoScroll = useCallback(() => {
    ...
    if (
      ...
      typeof chipScrollRef.current.releasePointerCapture === 'function'
    ) {
      try {
        chipScrollRef.current.releasePointerCapture(activePointerIdRef.current);
      } catch (err) {
        /* noop */
      }
    }
  ...
  const startCategoryAutoScroll = useCallback((direction) => {
    if (!direction) return;
    ...
    }, 200); // ~5 chips per second
  }, [stopCategoryAutoScroll]);

  const handleChipPointerDown = useCallback((event) => {
    ...
    longPressTimeoutRef.current = window.setTimeout(() => {
      startCategoryAutoScroll(direction);
    }, 2500);
    if (typeof chipScrollRef.current.setPointerCapture === 'function') {
      try {
        chipScrollRef.current.setPointerCapture(event.pointerId);
  ...
  const handleChipPointerEnd = useCallback((event) => {
    stopCategoryAutoScroll();
    if (
      activePointerIdRef.current !== null &&
      chipScrollRef.current &&
  ...
  useEffect(() => {
    ...
    const desiredScroll = targetOffset - (containerWidth - targetWidth) / 2;
    const clampedScroll = Math.max(0, Math.min(desiredScroll, container.scrollWidth - containerWidth));

    container.scrollTo({ left: clampedScroll, behavior: 'smooth' });
  }, [selectedCategory]);

  useEffect(() => () => stopCategoryAutoScroll(), [stopCategoryAutoScroll]);
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const handleScroll = () => {
      setHeaderElevated(window.scrollY > 10);
    };
    handleScroll();
  ...
  return (
    <div className="min-h-screen bg-white pb-20">
      <div className="mx-auto w-full max-w-[1280px] px-4 py-4">
        {/* Header: chevron, search, and categories */}
        <nav
          className={`sticky top-0 z-30 px-4 py-2 -mx-4 transition duration-200 backdrop-blur-lg ${
            headerElevated ? 'bg-white/62 shadow-sm' : 'bg-white/54'
          }`}
        >
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center justify-center px-3.5 py-[0.73rem] rounded-full text-sm font-medium bg-gray-100 text-gray-800 hover:bg-gray-200 transition-colors focus:outline-none focus:ring-2 focus:ring-orange-300 flex-shrink-0"
              title="Back to Admin"
            ...
            {searchMode ? (
              ...
              <>
                <button
                  ...
                  className="inline-flex items-center justify-center px-3.5 py-[0.73rem] rounded-full text-sm font-medium bg-gray-100 text-gray-800 hover:bg-gray-200 transition-colors flex-shrink-0 focus:outline-none focus:ring-2 focus:ring-orange-300"
                  title="Search"
                  aria-label="Search"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-4 w-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="chip-frame">
                    <div
                      className="horizontal-scroll items-center"
                      ref={chipScrollRef}
          ...
          <div>
            <div
              ...
              onTouchEnd={(event) => {
                ...
                if (currentIndex === -1 || totalCategories <= 0) return;
                if (dx < 0) {
