...
import React, { useEffect, useState, useMemo, useRef, useCallback, useTransition, lazy, Suspense } from "react";
import { useRouter } from "next/router";
import dynamic from 'next/dynamic';

// Lazy load heavy components
const LocalDataService = dynamic(() => import("@/src/localDataService"), { ssr: false });

// Lazy load data
const menuDataPromise = import("../data/menuData.json");
const localOrdersPromise = import("../data/local-orders.json");
...
export default function AdminPage() {
  ...
  const bulkOffChickenWithOverrides = useCallback(async ({ noConfirm = false, silent = false } = {}) => {
    ...
    try {
      ...
      console.error('bulkOffChickenWithOverrides', e);
      if (!silent) showChToast('Failed to turn OFF chicken items.', 'error');
    } finally {
      setBulkBusy(false);
    }
  }, [individualOverrides, fetchMenu, showChToast]);
  ...
  const [applyOffersActive, setApplyOffersActive] = useState(false);
  const applyOffersClickTimer = useRef(null);
  ...
  useEffect(() => {
    ...
    async function loadPending() {
      try {
        const c = await localData.countPendingChanges?.();
        if (mounted) setPendingCount(Number(c || 0));
      } catch (e) {
        console.warn('countPendingChanges failed', e);
      }
    }
    loadPending();

    const onUpdate = () => { loadPending(); };
    window.addEventListener('localData:update', onUpdate);
    return () => { mounted = false; window.removeEventListener('localData:update', onUpdate); };
  }, []);

  useEffect(() => {
    const handler = (e) => {
      // Prompt if there are pending changes
      if (pendingCount > 0) {
        e.preventDefault();
        e.returnValue = '';
      }
  ...
  async function handleSyncClick() {
    try {
      setSyncing(true);
      // ensure firebase initialized externally or pass config here
      const res = await localData.syncAllData();
  ...
  useEffect(() => {
    ...
    setRemoveForm((prev) => {
      ...
      const nextCategory = prev.category && availableCategories.includes(prev.category)
        ? prev.category
        : (availableCategories[0] || "");
      const items = nextCategory && Array.isArray(menu?.[nextCategory]) ? menu[nextCategory] : [];
      const names = items.map((item) => item?.name).filter(Boolean);
      const nextProduct = names.includes(prev.productName) ? prev.productName : (names[0] || "");
      if (nextCategory === prev.category && nextProduct === prev.productName) {
        return prev;
      }
      return { category: nextCategory, productName: nextProduct };
    });
  }, [showAddItem, categoryList, menu]);

  useEffect(() => {
    return () => {
      if (applyOffersClickTimer.current) {
        clearTimeout(applyOffersClickTimer.current);
      }
    };
  }, []);
  ...
  const fetchAndFilterOrders = useCallback(async () => {
    ...
    try {
      ...
      const [menuDataModule, localOrdersModule, LocalDataServiceModule] = await Promise.all([
        menuDataPromise,
        localOrdersPromise,
        import("@/src/localDataService")
      ]);
      
      const localData = LocalDataServiceModule.default || LocalDataServiceModule;
  ...
  const updateProductStock = async (category, productName, inStock, manualOverride) => {
    try {
      ...
      console.error(e);
      setToast(`Failed to update product stock: ${e.message}`);
    }
  };
  ...
  const handleApplyOffersClick = () => {
    setApplyOffersActive(true);
    if (applyOffersClickTimer.current) {
      clearTimeout(applyOffersClickTimer.current);
    }
    applyOffersClickTimer.current = setTimeout(() => {
      setApplyOffersActive(false);
    }, 400);
    router.push('/admin-offers');
  };

  const handleOpenProductModal = (mode = 'add') => {
    if (!menu || Object.keys(menu).length === 0) {
      fetchMenu();
    }
    const noCategories = categoryList.length === 0;
    const defaultCategory = noCategories ? "" : (addForm.category || categoryList[0] || "");
    const items = defaultCategory && Array.isArray(menu?.[defaultCategory]) ? menu[defaultCategory] : [];
    const firstProduct = !noCategories ? (items[0]?.name || "") : "";
    setAddBusy(false);
    setRemoveBusy(false);
    setAddForm({
      category: noCategories ? "" : defaultCategory,
      isNewCategory: noCategories,
      newCategory: "",
      name: "",
      price: "",
      inStock: true,
  ...
  const handleDeleteProduct = async (category, product) => {
    ...
    setProductBusy((prev) => ({ ...prev, [key]: true }));
    try {
      await localData.removeProduct(category, product.name);
      await fetchMenu();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const s = await localData.getSyncStatus();
        if (s && typeof s.enabled === 'boolean') setSyncEnabled(s.enabled);
      } catch (e) {
        // ignore
      }
    })();
  ...
  // Prompt on back/exit: when user attempts to close app from admin UI, ask to sync if disabled
  useEffect(() => {
    const beforeUnload = async (e) => {
      // If running inside a browser this prevents close; on Capacitor we also listen to back button elsewhere
      if (!syncEnabled) {
        const should = confirm('Sync is OFF. Sync data before exit? Press OK to sync now.');
        if (should) {
          e.preventDefault?.();
          setSyncBusy(true);
          try {
            await localData.syncAllData();
            await localData.setSyncStatus(true);
            setSyncEnabled(true);
          } catch (err) {
            // allow exit but inform
      ...
      {syncing && (
        <div className="fixed inset-0 z-[1100] flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-lg p-6 shadow-lg flex items-center gap-4">
            <svg className="w-6 h-6 animate-spin text-orange-500" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" strokeOpacity="0.25" fill="none"/><path d="M22 12a10 10 0 0 1-10 10" stroke="currentColor" strokeWidth="3" strokeLinecap="round" fill="none"/></svg>
            <div>
              <div className="text-sm font-medium">Syncing changes</div>
              <div className="text-xs text-gray-500">Please wait — syncing your pending changes</div>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="flex gap-2 overflow-x-auto flex-nowrap scrollbar-hide pr-2">
            ...
            <div className="flex-shrink-0 ml-auto mr-2 flex items-center justify-center" ref={menuButtonRef}>
              ...
              <button
                ...
                title="Sync now"
                className={`ml-2 px-3 py-1 rounded-full text-sm font-medium transition-colors ${syncing ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`}
              >
                {syncing ? (
                  <span className="inline-flex items-center gap-2"><svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" strokeOpacity="0.25" fill="none"/><path d="M22 12a10 10 0 0 1-10 10" stroke="currentColor" strokeWidth="3" strokeLinecap="round" fill="none"/></svg> Syncing</span>
                ) : (
                  <span className="inline-flex items-center gap-2">Sync {pendingCount > 0 && (<span className="text-xs bg-red-600 text-white px-2 py-0.5 rounded-full">{pendingCount}</span>)}</span>
                )}
              </button>
              </div>
            </div>
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    setSyncBusy(true);
                    await localData.setSyncStatus(next);
                    setSyncEnabled(next);
                    if (next) {
                      // optionally perform an immediate sync when enabling
                      await localData.syncAllData();
                    }
                  } catch (e) {
                    setToast('Failed to toggle sync');
                  } finally {
                    setSyncBusy(false);
                    setMenuOpen(false);
                  }
                }}
                className={`w-full text-left px-3 py-2 rounded-md text-sm font-medium mb-2 ${syncEnabled ? 'bg-orange-500 text-white hover:bg-orange-600' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`}
              >
                {syncBusy ? 'Syncing...' : (syncEnabled ? 'Sync: ON' : 'Sync: OFF')}
              </button>
              <button
        ...
        <div>
          ...
          <div className="w-full overflow-x-auto scrollbar-none" style={{ msOverflowStyle: 'none', scrollbarWidth: 'none' }}>
            <div className="mt-2 mb-4 flex items-center gap-2 min-w-0 w-max border-[0.4px] border-gray-400 rounded-full bg-white/80 px-4 py-1.25 shadow-sm whitespace-nowrap touch-pan-x scrollbar-none" style={{ msOverflowStyle: 'none', scrollbarWidth: 'none' }}>
              ...
              <button
                onClick={handleApplyOffersClick}
                className={`px-3.5 py-2 rounded-full text-sm font-medium transition-colors ${
                  applyOffersActive
