...
export default function AdminUnifiedPage() {
  ...
  const {
    ...
    updateStatus,
    updateLocalStatus,
  } = useAdminOrders(setOrders, setLocalOrders, setLoading, setFetching, fetchingRef);
  ...
  // Keyboard shortcuts
  useAdminKeyboardShortcuts(handleTabChange, showToast, expandedOrderId, getFilteredOrders(orders, localOrders, tab));
  ...
  const filteredOrders = getFilteredOrders(orders, localOrders, tab);
    ...
    >
      {tab === "Products" ? (
        ...
        <OrdersTab
          ...
          onUpdateLocalStatus={(id, status) => updateLocalStatus(id, status, localOrders, setLocalOrders, showToast)}
          now={now}
        />
      )}
